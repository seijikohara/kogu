...
interface GenerateTabTemplateProps {
	readonly input: string;
	readonly onInputChange: (value: string) => void;
	readonly onStatsChange?: (stats: TabStats) => void;

	// Per-format input handling. `parseInput` throws on parse error;
	// `validateInput` returns `null` for empty input, `true` / `false` otherwise.
	readonly parseInput: (input: string) => unknown;
	readonly validateInput: (input: string) => boolean | null;

	// Per-format UI strings + editor mode.
	readonly inputEditorMode: EditorMode;
	readonly inputTitle: string;
	readonly inputPlaceholder: string;
	readonly emptyTitle: string;

	// Value `useClipboardActions` writes back on clear. JSON uses `'{}'` so a
...
// Shared template for the formatter-family Generate tab. Owns every piece of
...
export type { GenerateTabTemplateProps, TabStats };
