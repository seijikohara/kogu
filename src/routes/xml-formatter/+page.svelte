<script lang="ts">
	...
	const liveStats = $derived.by((): XmlStats | null => {
		const input = sharedInput.trim();
		if (!input) return null;
		try {
			return calculateXmlStats(input);
		} catch {
			return null;
		}
	});
	...
	{#snippet statusContent()}
		{#if liveStats}
			<span class="text-muted-foreground"
				>Elements: <strong class="text-foreground">{liveStats.elements}</strong></span
			>
			<span class="text-muted-foreground"
				>Attributes: <strong class="text-foreground">{liveStats.attributes}</strong></span
			>
			<span class="text-muted-foreground"
