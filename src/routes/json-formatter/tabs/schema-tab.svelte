<script lang="ts">
	...
	const inferredSchema = $derived.by(() => {
		if (!schemaInput.trim() || schemaInput.trim() === '{}') {
			return '';
		}
		try {
			return JSON.stringify(inferJsonSchema(schemaInput), null, 2);
		} catch {
			return '';
		}
	});
	...
	const handleValidateSchema = () => {
		if (!schemaInput.trim() || !schemaDefinition.trim()) {
			schemaError = 'Please enter JSON and a schema';
			return;
		}
		try {
			const data = JSON.parse(schemaInput);
